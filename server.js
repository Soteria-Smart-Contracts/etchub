...
// ONE-TIME MIGRATION: Copy all articles from generated_news to database on startup
const migrateArticlesToDatabase = async () => {
    try {
        ...
        for (const file of files) {
            ...
            try {
                await storage.saveArticle(slug, title, author, description, content, 'news');
                migrated++;
            } catch (err) {
        ...
        if (migrated > 0) console.log(`Migration: ${migrated} articles synced to database`);
    } catch (err) {
        console.error('Migration error:', err.message);
    }
};

migrateArticlesToDatabase();
